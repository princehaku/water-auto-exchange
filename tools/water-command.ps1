...
function Invoke-WaterCommand([string]$Value) {
    ...
    throw "No water controller response on $Port. Confirm version 0.7.0/0.7.1/0.7.2/0.7.3/0.7.4/0.7.5/0.7.6/0.7.7/0.8.0/0.8.1/0.8.2/0.8.3 is flashed and the USB user port is available."
}
...
try {
    ...
    if ($status -notmatch '^OK STATUS project=water_auto_exchange version=(?:0\.7\.[01234567]|0\.8\.[0123])(?: |$)') {
        throw 'The selected port did not identify as water_auto_exchange version 0.7.0/0.7.1/0.7.2/0.7.3/0.7.4/0.7.5/0.7.6/0.7.7/0.8.0/0.8.1/0.8.2/0.8.3.'
    }
    Write-Output $status
    if ($Command -in @('FILL_OFF', 'DRAIN_OFF') -and ($status -notmatch 'version=0\.8\.[0123](?: |$)' -or $status -notmatch '(?:^| )control_mode=manual(?: |$)')) { throw 'Separate OFF commands require firmware 0.8.0/0.8.1/0.8.2/0.8.3 in manual mode.' }
