...
用户明确实板 0.8.2 已投入生产，后续尽可能只改网页和服务端，并追加要求仓库中的板端代码也回滚到0.8.2，等以后确有需要再改。固件源码与生成包恢复至已发布的`3020034`版本，LuaTools使用`water-online-0.8.2`；本轮不刷板。0.8.3的板端正常超时改动回滚，未发布的0.8.4方案撤销，当前Web功能均兼容生产0.8.2。用户测量排水300秒下降约10个百分点，对应线性满到空用时约3000秒（50分钟），不是1000秒；补水用时须独立测量，当前水位须按现场校准，不能自行设为满水或覆盖用户配置。
...
为满足用户此前“正常超时只关闭、不锁故障”的要求，服务端兼容0.8.2在普通手动180/300秒软上限时发送正常STOP，须本轮成功停止回执及全关IDLE才能清理计时，失败仍终止连接走固件5秒保护。它不发送RESET、不清除真实FAULT；旧/迟到OFF回执不能代替本轮关闭。0.8.3原有内部到时命令仍兼容。
