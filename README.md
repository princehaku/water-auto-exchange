...
> 2026-09-13：生产实板保留 **0.8.2**。主界面补水、冲水每次按该路完整100%校准时长运行，服务端自动分轮；两路可同时运行，再点对应开关结束该路作业。整次进度跨轮累计，补水每轮最多170秒、排水每轮最多290秒，确认本路关闭后间隔2秒续下一轮；断联或停止后不会自动续开。另保留“按目标水位”入口。仓库固件源码和生成包均保持已投产的0.8.2，无需刷板。
