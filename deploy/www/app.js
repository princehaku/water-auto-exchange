...
function can(command){
  ...
  return ['FILL','DRAIN'].includes(command)&&['0.7.0','0.7.1','0.7.2'].includes(d.version)&&d.control_mode==='manual'&&d.ready==='1'&&d.outputs_known==='1'&&d.overflow==='0'&&['IDLE','DONE'].includes(d.state);
}
...
function render(data){
  ...
  $('control-hint').textContent=!online?'设备离线，连接恢复后可操作。':!['0.7.0','0.7.1','0.7.2'].includes(d.version)?'更新设备后可使用手动开关。':d.control_mode!=='manual'?'设备需配置为手动开关模式。':d.state==='UNCONFIGURED'?'输出引脚尚未配置。手动开关不需要水位传感器。':d.state==='FAULT'?'排除故障后复位；停止输出仍可使用。':['FILLING','DRAINING'].includes(d.state)?'请先关闭当前输出，再打开另一路。':'点击开关打开或关闭，不等待水位信号。';
