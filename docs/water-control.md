...
联网手动控制由Web服务端执行补水180秒（3分钟）、排水300秒（5分钟）的软上限，从该路开启授权开始计时，不等待ON回执才开始。重复开启、开关另一路、刷新或关闭网页均不重置计时；水位未校准或估算不确定也不影响此计时。任一路到期由服务端内部关断命令触发全关并锁存FAULT，需复位后重新开启；没有关断确认时终止会话，不持续发送心跳为输出续期。
...
USB本地命令及自动液位模式保留固件180/300秒上限。0.8.2远程手动控制使用WSS，不支持旧HTTP到USB网关代替这套机制。GPIO12在任意一路开启时常亮；联网待机每次有效心跳回执亮200ms、正常30秒一次；未联网或连接失败时恢复原网络闪烁。
...
源码及生成包0.8.2，8Lua+CA，原CORE与默认库不变，见[Web与4G接入](web-console.md)。本轮未刷写或以0.8.2操作真实输出；用户下载后新保护才可在实板生效。用户当前只要求软件开关，不以水流测量作为本阶段前提。
...
本地验证覆盖并行开关、服务端180/300秒软上限、独立5秒失联关断、重复开启不延期和端到端命令交付。GPIO测试均为模拟；最近实板上报为0.8.0，新机制需要下载0.8.2后生效。实际泵、探针测试不在本阶段范围内。
