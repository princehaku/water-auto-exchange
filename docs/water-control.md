...
USB本地命令及自动液位模式保留固件180/300秒上限，正常补排水到时同样关闭全部并回到IDLE。0.8.3远程手动控制使用WSS，不支持旧HTTP到USB网关代替这套机制。GPIO12在任意一路开启时常亮；联网待机每次有效心跳回执亮200ms、正常30秒一次；未联网或连接失败时恢复原网络闪烁。
...
源码及生成包0.8.3，8Lua+CA，原CORE与默认库不变，见[Web与4G接入](web-console.md)。本轮未刷写或以0.8.3操作真实输出；用户下载后新保护才可在实板生效。用户当前只要求软件开关，不以水流测量作为本阶段前提。
...
`drain_timeout`、`fill_timeout` 在0.8.3只表示到时自动关闭，回到IDLE即可直接操作，无需RESET；已有FAULT不会因此解除。`overflow`、通信中断期限、传感器读取错误等真实故障仍锁存为 `FAULT`。关断失败时两个输出都会独立尝试关闭，不能仅凭 STOP 应答代替电压确认。轮询定时器失效后禁止重新 START/FILL/DRAIN，修复问题后重启应用；普通传感器故障则可恢复稳定后 RESET。

## 下载与验证边界

LuaTools选择water-online-0.8.3项目，使用build/firmware/flash-files.txt中的完整8个Lua文件和1个CA证书，保留既有CORE、默认库及USB trace。准备与验证步骤见[Web与4G接入](web-console.md)。旧water-exchange的5文件清单不包含网络功能，不能用于当前联网版本。由用户点击下载脚本；此前Escape停止的桌面输入未恢复。

本地验证覆盖并行开关、服务端180/300秒软上限、独立5秒失联关断、重复开启不延期和端到端命令交付。GPIO测试均为模拟；最近实板上报为0.8.2，新机制需要下载0.8.3后生效。实际泵、探针测试不在本阶段范围内。
