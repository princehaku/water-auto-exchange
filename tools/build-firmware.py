...
FILES = ('main.lua', 'water_config.lua', 'water_cycle.lua', 'water_control.lua',
         'water_usb.lua', 'water_network.lua', 'water_network_config.lua')
...
def main():
    ...
    template = (ROOT / 'src/water_network_config.lua').read_text(encoding='utf-8')
    template = template.replace('enabled = false', 'enabled = true')
    template = template.replace('"https://bytegallop.com/water"', json.dumps(config['url'].rstrip('/')))
    ...
    print('Prepared 7 Lua files and 1 CA certificate in build/firmware; GPIO mapping remains unchanged.')
