...
## 2026-09-13 最新：0.8.3 补排水到时自动关闭，不锁存故障
...
手动模式没有真实水位传感器。新页面默认鱼缸水面是**场景示意**，不显示百分比。用户需根据过去实际经验填写“空到满”和“满到空”的时长，并在设备在线、两路均关闭且现场水位已知时设置当前水位（满水 100%、空水 0% 或已知百分比）。校准保存在服务器 SQLite，而非单个浏览器；之后服务端按设备上报的各路开启时间积分，两路同时开启时按净变化计算，并在各浏览器显示同一模拟水位。首次安装前的历史命令没有完整输出变化时间戳，不能倒推出真实水位或自动回填校准。工作期间通信断开、上报超过 12 秒或服务重启后输出时刻不明，估算标为不确定，需重新查看现场水位再校准。水位模拟不参与物理控制；补排水保护计时独立于水位估算，0.8.2/0.8.3联网手动控制由服务端限时，更早版由板端限时。
...
主页显示每路本次估算水量、净流量，以及按当前开关保持状态推算的满水或排空时间。两路同时开启分别累计补水与排水，净流量取两者之差；净变化为零时不显示满空倒计时。满水/排空的预计时间不安排停止；每路补排水保护计时独立执行，不受水位估算影响。达到 0%/100% 只截断缸内水位，阀门本次累计仍按已确认运行区间增加，不能把累计注入量当成缸内现存量。
...
所有累计只来自连续、可信的设备输出报告，浏览器不自行积分，也不从控制命令的提交时间补算。工作中失联、输出未知或上报断档时暂停；服务连接中断时保留上次水量并隐藏实时净流量和预计时间。估算不确定后需核实现地水位重新校准。本轮只统计阀门补水和排水，不包括自然蒸发、渗漏或缸内自循环。
