...
## 2026-09-14：边界停止与部署空闲检查

Web手动与自动控制按可信估算0%/100%边界停止，所有服务端作业统一60秒分轮。固件仍为0.8.2。网页确认弹窗、全程预估、桌面分区和任务阶段倒计时一并更新；手动完整校准用时仅保留安全上限，不能越过已知水位边界继续运行。

`deploy/install.sh`在镜像构建完成、停止旧容器前调用`deploy/check-idle.py`。必须读取到设备在线、双路确认关闭、没有运行中自动/手动作业、没有待确认命令与开启授权，才继续更新。检查失败在安装回滚处理启用前退出，保留现有控制服务；首装没有旧容器时跳过。检查使用既有管理凭据读取状态，不发送设备控制或校准命令。

发布前218项Python、真实本地HTTP/WS换水与边界联调、三个受影响浏览器兼容分支和11种桌面/手机布局均通过；固件源码无改动。只读生产为0.8.2在线IDLE、双OFF、无运行作业及待确认命令，保留校准fill60/drain400/capacity5、level100、uncertain=false。上述水位为服务器估算。

已部署，备份`/apps/water-auto-exchange/backups/20260913T200517Z-298168`。公网9项静态资源与3个服务端源文件匹配本地；WSS诊断probe、water/sms健康、匿名status401及缺失资源404通过。容器healthy/restart=0，StartedAt为2026-09-13T20:05:29.260474885Z；实板已重新连接，0.8.2在线IDLE双OFF，校准与可信水位估算保持不变。未刷板或执行生产控制、校准命令。
...
| 数据库 | `data/water.db`，容器挂载到 `/data`；包含操作记录、999天管理员会话摘要、连接历史、最近设备快照与累计流量计数 |
