...
## 2026-09-13 最新：0.8.1 补水3分钟、排水5分钟与倒计时
...
顶部只有一个设备概况指示器：在线、就绪、输出已知且无故障/溢水时显示正常；设备离线、故障、未就绪或输出未知显示异常；浏览器与服务失去连接时显示“状态未知”，不会误认设备离线。手机将两个开关并排、鱼缸与控制上下排列；桌面并列显示，主界面按可用视口高度排布。0.8.0/0.8.1 的 manual 模式支持两路同时开启、分别关闭，旧固件保持互锁及 STOP 语义。开关仍以设备回执为准。
...
手动模式没有真实水位传感器。新页面默认鱼缸水面是**场景示意**，不显示百分比。用户需根据过去实际经验填写“空到满”和“满到空”的时长，并在设备在线、两路均关闭且现场水位已知时设置当前水位（满水 100%、空水 0% 或已知百分比）。校准保存在服务器 SQLite，而非单个浏览器；之后服务端按设备上报的各路开启时间积分，两路同时开启时按净变化计算，并在各浏览器显示同一模拟水位。首次安装前的历史命令没有完整输出变化时间戳，不能倒推出真实水位或自动回填校准。工作期间通信断开、上报超过 12 秒或服务重启后输出时刻不明，估算标为不确定，需重新查看现场水位再校准。模拟值不参与物理控制，也不改动板端的补排水超时保护。
...
主页显示每路本次估算水量、净流量，以及按当前开关保持状态推算的满水或排空时间。两路同时开启分别累计补水与排水，净流量取两者之差；净变化为零时不显示满空倒计时。预计时间不安排自动停止，也不会绕过当前固件的每路限时保护。达到 0%/100% 只截断缸内水位，阀门本次累计仍按已确认运行区间增加，不能把累计注入量当成缸内现存量。
