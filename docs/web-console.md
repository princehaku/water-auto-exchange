...
主页显示每路本次估算水量、净流量，以及按当前开关保持状态推算的满水或排空时间。两路同时开启分别累计补水与排水，净流量取两者之差；净变化为零时不显示满空倒计时。预计时间不安排自动停止，也不会绕过每路 120 秒保护。达到 0%/100% 只截断缸内水位，阀门本次累计仍按已确认运行区间增加，不能把累计注入量当成缸内现存量。
