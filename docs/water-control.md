...
生产设备继续使用0.8.2，后续优先修改Web端。新版服务端在180/300秒正常软上限发送普通STOP，并确认全部关闭，以兼容0.8.2且避免正常超时故障锁定；仓库固件源码及生成包也已回滚到0.8.2，生产实板无需重新下载。网页“按目标水位”作业按校准估算分轮补排水，单轮最多补170秒/排290秒，确认OFF后间隔2秒继续；设备断联、手动停止或估算不确定时结束，不自动恢复。具体校准和任务说明见[Web控制台](web-console.md)。
...
两路都开时状态为EXCHANGING；网页显示“补水与排水同时进行”，两个开关分别发送对应OFF命令。保留停止全部输出按钮。0.8.0及之后的手动模式支持并行；更旧固件保持互锁和STOP关闭行为。

联网手动控制由Web服务端执行补水180秒（3分钟）、排水300秒（5分钟）的软上限，从该路开启授权开始计时，不等待ON回执才开始。重复开启、开关另一路、刷新或关闭网页均不重置计时；水位未校准或估算不确定也不影响此计时。任一路到期由服务端内部关断命令触发全关并回到IDLE，保留到时原因，可以直接重新开启，不锁存故障、不需要RESET；没有关断确认时终止会话，不持续发送心跳为输出续期。

固件只有在服务端确认软上限能力后才允许远程手动输出跳过本地180/300秒时限。有效通信超过5秒未续期时，由独立控制定时器关断并锁存communication_timeout；显式断线仍立即停止。重复/过期pong、普通received消息、重复ON和另一路开启不能延长已有通信期限。溢水输入或IO异常继续关断全部；重连不恢复输出。

USB本地命令及自动液位模式保留固件180/300秒上限，板端自身的超时仍按0.8.2锁存FAULT；网页正常软上限通过普通STOP关断而避免触发该路径。0.8.2远程手动控制使用WSS，不支持旧HTTP到USB网关代替这套机制。GPIO12在任意一路开启时常亮；联网待机每次有效心跳回执亮200ms、正常30秒一次；未联网或连接失败时恢复原网络闪烁。
...
源码及生成包0.8.2，8Lua+CA，原CORE与默认库不变，见[Web与4G接入](web-console.md)。本轮仅恢复同版本源码和包，不刷写或操作真实输出；当前Web功能无需重刷生产实板。用户当前只要求软件开关，不以水流测量作为本阶段前提。
...
stateDiagram-v2
    ...
    DRAINING --> IDLE: 排水到时自动关闭
    DRAINING --> FAULT: 溢水或IO异常
    FILLING --> IDLE: 补水到时自动关闭
    FILLING --> FAULT: 溢水或IO异常
...
读取液位每 100ms 一次，正常液位转换防抖 500ms，排水转补水间隔 1000ms。当前默认排水超时为300000ms、补水超时为180000ms，由用户分别指定5分钟和3分钟。软件的停机响应依赖 Lua 事件循环正常运行，不能代替独立硬件限位。
...
0.8.2板端自身的`drain_timeout`、`fill_timeout`仍锁存FAULT。新版Web服务端正常到时发送普通STOP，确认IDLE后可直接操作，无需RESET；这不会解除已有FAULT。`overflow`、通信中断期限、传感器读取错误等真实故障仍锁存为 `FAULT`。关断失败时两个输出都会独立尝试关闭，不能仅凭 STOP 应答代替电压确认。轮询定时器失效后禁止重新 START/FILL/DRAIN，修复问题后重启应用；普通传感器故障则可恢复稳定后 RESET。

## 下载与验证边界

LuaTools选择water-online-0.8.2项目，使用build/firmware/flash-files.txt中的完整8个Lua文件和1个CA证书，保留既有CORE、默认库及USB trace。准备与验证步骤见[Web与4G接入](web-console.md)。旧water-exchange的5文件清单不包含网络功能，不能用于当前联网版本。由用户点击下载脚本；此前Escape停止的桌面输入未恢复。

本地验证覆盖并行开关、服务端180/300秒软上限、独立5秒失联关断、重复开启不延期和端到端命令交付。GPIO测试均为模拟；最近实板上报为0.8.2，本轮Web改动已兼容该版本，无需下载新固件。实际泵、探针测试不在本阶段范围内。
