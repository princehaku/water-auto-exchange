> 当前源码0.7.1，默认手动开关，不要求传感器或水流验证。后面的液位流程仅用于未来automatic模式。
...
网页开关和设备回报同步，按同一开关第二次会发送STOP。此阶段只验收软件开关和命令逻辑，不要求实际水流验证。当前生成包是0.7.1，下载清单仍为8Lua+CA，见[Web与4G接入](web-console.md)。
