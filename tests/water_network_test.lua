...
local function fixture()
    ...
    f.config = {enabled=true, url="wss://example.test/water/api/device/ws",device_key=string.rep("k",32),ca_cert="water-ca.crt",
        heartbeat_ms=30000,active_heartbeat_ms=2000,offline_stop_ms=10000,idle_timeout_ms=75000}
    f.controller = {
    ...
    f.deps={tick=function() return f.tick end,
        ...
        usb={format_status=function() return "project=water_auto_exchange version=0.5.0 state="..f.state end},
...
test("idle sends no two second polls",function() local f=fixture();f.connect();for i=1,29 do f.advance(1000) end;assert(#f.sent==1);f.advance(1000);assert(f.last().type=="ping") end)
...
test("active heartbeat interval is two seconds",function() local f=fixture();f.connect();f.offer("START");f.execute("START");f.advance(2000);assert(f.last().type=="ping") end)
