...
function Invoke-WaterCommand([string]$Value) {
    ...
    throw "No water controller response on $Port. Confirm version 0.7.0/0.7.1/0.7.2/0.7.3/0.7.4 is flashed and the USB user port is available."
}
...
try {
    ...
    if ($status -notmatch '^OK STATUS project=water_auto_exchange version=0\.7\.[01234](?: |$)') {
        throw 'The selected port did not identify as water_auto_exchange version 0.7.0/0.7.1/0.7.2/0.7.3/0.7.4.'
    }
    Write-Output $status
