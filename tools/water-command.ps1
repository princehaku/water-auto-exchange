...
function Invoke-WaterCommand([string]$Value) {
    ...
    throw "No water controller response on $Port. Confirm version 0.6.0 is flashed and the USB user port is available."
}
...
try {
    ...
    if ($status -notmatch '^OK STATUS project=water_auto_exchange version=0\.6\.0(?: |$)') {
        throw 'The selected port did not identify as water_auto_exchange version 0.6.0.'
    }
    Write-Output $status
