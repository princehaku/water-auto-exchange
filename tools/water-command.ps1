...
function Invoke-WaterCommand([string]$Value) {
    ...
    throw "No water controller response on $Port. Confirm version 0.7.0/0.7.1/0.7.2/0.7.3/0.7.4/0.7.5 is flashed and the USB user port is available."
}
...
try {
    ...
    if ($status -notmatch '^OK STATUS project=water_auto_exchange version=0\.7\.[012345](?: |$)') {
        throw 'The selected port did not identify as water_auto_exchange version 0.7.0/0.7.1/0.7.2/0.7.3/0.7.4/0.7.5.'
    }
    Write-Output $status
