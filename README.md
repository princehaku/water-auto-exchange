...
最近验证：`0.2.4` 的 13 路诊断曾通过 `STATUS` 核对，并于 2026-09-06 00:23:21.084 成功 `STOP`。用户随后提供 00:31:47.904 的 13 路扫描日志并确认 GPIO12 对应板载灯。本轮读取状态时 COM4 不存在，尚未重新核实当前运行版本和状态。电机业务映射仍未完整确认，原定通电 3 秒/断电 3 秒尚未启用。
...
用户已澄清：只循环切换 GPIO5，供其测量板上其他测试点；高、低各 5 秒。`0.2.5` 已准备完成（电机/USB/启动测试 18/18、诊断测试 13/13、官方 LuaFLOAT 语法及 PowerShell 解析通过），仅控制 GPIO5（物理 49 脚，固定 1.8 V 域），按 HIGH 5 秒/LOW 5 秒自动循环，周期 10 秒；不操作其他 GPIO、任何 LDO 或物理 UART。上电自动开始，`STOP` 仍可停止，下次上电再次自动开始。继续保持外接负载断开。待准备完成通知后由用户手动下载脚本，当前 `0.2.5` 尚未实测；原 13 脚 `0.2.4` 已停止。测量依据和版本记录见 [电机控制推断](docs/board-control-analysis.md)。
...
GPIO12 记录来自 2026-09-06 00:31:47.904 的 `gpio=12 physical=53 level=1 phase=HIGH index=5/13`。此记录不改变 `0.2.5` 只循环 GPIO5 的程序范围。
...
| 当前程序 | `gk21_motor_test`，已实测的 `0.2.4` 已成功 `STOP`；仅 GPIO5 的 `0.2.5` 已准备完成、待用户下载验证；电机业务配置保持 `enabled=0`、`mapping_confirmed=0` |
...
该版本电机/USB/启动测试 18/18、诊断测试 19/19、官方 LuaFLOAT 语法检查及 PowerShell 解析均通过。以下保留 13 脚版本的历史行为；最新目标为文首所述仅 GPIO5 的 `0.2.5`，尚未刷入验证。
