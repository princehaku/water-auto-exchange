...
local function fixture(real_controller)
    ...
    _G.PROJECT, _G.VERSION = "water_auto_exchange", "0.7.5"
    _G.uart = {
...
test("fragmented STATUS is read-only and unknown water level is explicit", function()
    ...
    equal(#f.replies, 1)
    f.feed("TUS\r")
    contains(f.replies[2], "OK STATUS project=water_auto_exchange version=0.7.5")
...
test("boot prints immediately and every 5 seconds without starting outputs", function()
    ...
    equal(f.calls.fill, nil)
    equal(f.gpio_calls, 0)
    contains(table.concat(f.logs), "WATER STATUS project=water_auto_exchange version=0.7.5")
    equal(#f.timers, 1)
