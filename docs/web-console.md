...
新入口为 `/water/aquarium.html`，原 `/water/` 控制台继续保留并互相提供入口。新页面在浏览器本地渲染透明鱼缸、两只红耳巴西龟与一小块露出水面的陆地；二维操作区有补水、冲水两个独立开关、各路运行时间与 120 秒上限进度、设备在线状态、最近通信、状态说明，以及可展开的最近 60 条操作/在线离线记录。登录会话和控制 API 与旧页面共用，页面可见时每 2 秒自动同步。仅 0.8.0/manual 可同时开启并分别关断；旧固件仍按互锁及 STOP 语义。开关状态仍以设备上报为准，不因命令入队而提前显示开启。
